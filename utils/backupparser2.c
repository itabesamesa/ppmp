...
polygon arraytopolygon(localvarlist variables, unsigned int i) {
	...
	for(unsigned int i = 0; i < p.corners; i++) {
		p.p[i] = arraytoxy_int(variables, variables.v[i].avi[i]);
	}
	return p;
}

pval* arraytocolor(localvarlist variables, unsigned int i) {
	color c;
	c.type = variables.v[variables.v[i].avi[0]].num;
	c.c = malloc(color_to_depth(c.type)*sizeof(pval));
	for(unsigned int a = 1; a < variables.v[i].avilen; a++) array[a] = variables.v[variables.v[i].avi[a]].num;
	return array;
}
	...
	while(len < end) {
		...
		switch(type) {
			...
			default:
				...
				short hasindex = 0;
				if(varname[offset-1] == ']') {
					int bracketstart = getstart(varname, '[', offset);
					if(bracketstart == 0) {
						err.type = 7;
						return err;
					}
				...
				if(line[len] == '$') { //fucky malloc stuff at circle_mask
					err.uv++;
					printf("var\n");
					...
					printf("var\n");
					realloc_defvarerr_vi(&err, err.len+1);
					if(variables->v[foundvar].type == 3 && hasindex == 1) {
...
error interprate(text t) {
	func functions[35] = { 
		...
		{"selection_mask",                  14,       2,          8, 1, 0},
		{"selection_mask_rgb",              18,       2,          8, 1, 0},
		//./utils/display.h
		//./utils/pngtopng.h
		{"openimg",                         7,        1,          1, 1, 0},
		//./utils/wm.h
	...
	for(int linenum = 1; linenum <= t.lines; linenum++) {
		...
		if(end > 1) {
			printf("line: |%.*s|  %d\n", end, line, end);
			if(line[0] == '$') { //doesn't consider there being spaces infront of '$'
				line = &line[1];
				...
				switch(selectedfunction) { //--startswitch--
					...
					case 5:
						...
						images.list[outputimg] = worley_noise_2d(arraytoxyz_int(variables, options.v[0]), variables.v[options.v[1]].num, variables.v[options.v[2]].num);
						break;
					case 6:
						printf("perlin_noise\n");
						images.list[outputimg] = perlin_noise(arraytoxyz_int(variables, options.v[0]), variables.v[options.v[1]].num, variables.v[options.v[2]].num);
						break;
					case 7:
						printf("box_sample\n");
...
////variables.v[options.v[0]].num, variables.v[options.v[1]].num, variables.v[options.v[2]].num);
					...
					case 15:
						printf("empty_img\n");
						images.list[outputimg] = empty_img(variables.v[options.v[0]].num, variables.v[options.v[1]].num, variables.v[options.v[2]].num);
						break;
					case 16:
						printf("empty_colored_img\n");
						images.list[outputimg] = empty_colored_img(variables.v[options.v[0]].num, variables.v[options.v[1]].num, variables.v[options.v[2]].num, arraytopvalarray(variables, options.v[3]));
						break;
					case 17:
					...
					case 19:
						...
						images.list[outputimg] = generate_box_sample_blur_kernel(variables.v[options.v[0]].num, variables.v[options.v[1]].num);
						break;
					case 20:
						printf("generate_gaussian_blur_kernel\n");
						images.list[outputimg] = generate_gaussian_blur_kernel(variables.v[options.v[0]].num, variables.v[options.v[1]].num, variables.v[options.v[2]].num);
						break;
					case 21:
					...
					case 23:
						printf("apply_mask\n");
						images.list[outputimg] = apply_mask(images.list[variables.v[options.v[0]].img], images.list[variables.v[options.v[1]].img], arraytoxyz_int(variables, options.v[2]), variables.v[options.v[3]].num);
						break;
					case 24:
						printf("selection_mask_between\n");
						//images.list[outputimg] = selection_mask_between(images.list[variables.v[options.v[0]].img], arraytopvalarray(variables, options.v[1]), arraytopvalarray(variables, options.v[2]));
						break;
					case 25:
						printf("selection_mask_distance\n");
						//images.list[outputimg] = selection_mask_distance(images.list[variables.v[options.v[0]].img], arraytopvalarray(variables, options.v[1]), variables.v[options.v[2]].num);
						break;
					case 26:
						printf("selection_mask\n");
						//images.list[outputimg] = selection_mask(images.list[variables.v[options.v[0]].img], variables.v[options.v[1]].num);
						break;
					case 27:
						printf("selection_mask_rgb\n");
						//images.list[outputimg] = selection_mask_rgb(images.list[variables.v[options.v[0]].img], variables.v[options.v[1]].num);
						break;
					case 28:
						printf("openimg |%s|\n", variables.v[options.v[0]].string);
					...
					case 31:
						printf("div_clip\n");
						images.list[outputimg] = div_clip(images.list[variables.v[options.v[0]].img], variables.v[options.v[1]].num);
						break;
					case 32:
						printf("sobel_accumulator_func\n");
////arraytopvalarray(variables, options.v[0]), arraytopvalarray(variables, options.v[1]), variables.v[options.v[2]].num, variables.v[options.v[3]].num);
						break;
					case 33:
